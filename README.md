...
```bash
git clone https://github.com/kimsonga-data/data-analysis-shared.git ~/Documents/works/analysis-shared
```

> 경로는 자유롭게 설정해도 되지만, 팀 내에서 통일하면 CLAUDE.md를 그대로 공유할 수 있다.

### 2. Claude Code에 연결

**방법 A — 모든 프로젝트에 전역 적용** (권장)

`~/.claude/CLAUDE.md` 파일을 열고 맨 위에 추가:

```markdown
@/Users/{내_계정}/Documents/works/analysis-shared/CLAUDE.md
```

**방법 B — 특정 프로젝트에만 적용**

해당 프로젝트의 `CLAUDE.md` 맨 위에 추가:
