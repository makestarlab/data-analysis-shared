...
**제외 범위:**
- ✅ 제외해야 하는 분석: B2C 일반 유저 행동 분석 (생애주기, 이탈, 팬덤 분석, 유저 세그먼트 등)
- ❌ 제외하면 안 되는 분석: GMV·PU·ARPPU 등 **KPI 집계** (구매대행도 실제 매출이므로 포함)
...
**신규 구매대행 탐지 프로세스:**
1. `distinct_ip >= 30` 유저 중 목록에 없는 계정 확인 (쿼리: `queries/patterns.md` 참조)
2. B2C 구매대행으로 판명되면 해당 유저의 **이메일 도메인** 확인
3. 동일 도메인을 가진 다른 유저 검색 → 구매대행 가능성 높으면 목록에 추가
