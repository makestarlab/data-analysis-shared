...
## 쇼핑 채널 (biz_type = '쇼핑') 특성

`biz_type = '쇼핑'`은 이벤트와 무관하게 상시 판매되는 B2C 커머스 채널.
식별자: `biz_type = '쇼핑'`, `product_type IN ('N','M')`, `order_no LIKE 'C%' OR 'S%'`

### 기초통계 (최근 36개월, 2023-06 ~ 2026-06 기준)
...
| 평균 주문금액 | 53,168원 |
| 중간값 주문금액 | 28,200원 |
| 채널 | 100% 메이크스타웹 (위챗·오프라인 없음) |

### 유저 특성

| 구분 | 내용 |
|---|---|
| 1회 구매 후 이탈 | 61% — 재구매율이 구조적으로 낮음 |
| 상위 1.8%(100만원+) | GMV의 28% 기여 |
| 이벤트 리워드 중복 구매 | 37% — 나머지 63%는 쇼핑 전용 유저 |
| 주요 국가 | KR 39%, US 14%, TW 6%, JP 5% |

### 순수 쇼핑 수요 vs POB 포함

- `event_id IS NULL` (95%): 순수 쇼핑 주문
- `event_id IS NOT NULL` (5%): POB(특전) 응모 목적으로 쇼핑 상품을 구매한 주문
- 순수 쇼핑 수요 분석 시 `event_id IS NULL` 필터 권장

### 이벤트-쇼핑 재고 전환 패턴

이벤트 종료 후 동일 SKU가 쇼핑 채널에 등록되는 패턴이 존재 (OMS 기준 확인).

| 채널 | 이벤트 종료 후 1~30일 등록 SKU | 특성 |
|---|---|---|
| **B2B** | ~765개 SKU, 수량 수천~수만 개 | 계획된 도매 납품 프로세스로 이미 정착 |
| **B2C 쇼핑** | ~100개 SKU, 수량 10~220개 수준 | 소규모, 포카 단품 위주 |

> B2C 기준: 이벤트 종료 후 **7일 이내** 등록 시 수요가 가장 높음. 14일 이후 급감.
> B2B가 재고 소화의 주력 채널이며, B2C 쇼핑 채널 확대는 아직 미개척 영역.

### 주요 아티스트 (GMV 기준, 최근 36개월)

스트레이 키즈 1위(46억). tvN·U+tv 드라마 IP가 10~20위권 진입 — 비아이돌 IP의 쇼핑 채널 유효성 확인됨.

```sql
-- 쇼핑 기초 필터
WHERE biz_type = '쇼핑'
  AND market_type IN ('B2C','B2B')
  AND DATE(pay_date) >= DATE_SUB(CURRENT_DATE(), INTERVAL 36 MONTH)
